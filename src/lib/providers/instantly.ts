 ...
 * and an opaque session id (see /src/app/api/mailboxes/*).
 */
...
export type InstantlyOAuthSessionState = "pending" | "connected" | "failed";
...
function authHeaders(): Record<string, string> {
  ...
  if (!key) throw new ProviderError("instantly", "Instantly is not configured.");
...
export const instantlyProvider = {
  ...
  async initOAuth(provider: InstantlyMailboxProvider, redirectUrl: string): Promise<InstantlyOAuthInit> {
    const data = await fetchJson<{ session_id: string; authorization_url: string }>(
    ...
    return { sessionId: data.session_id, authorizationUrl: data.authorization_url };
  },

  /** Polls the state of a previously-started OAuth session. */
  async getOAuthSessionStatus(sessionId: string): Promise<InstantlyOAuthSessionStatus> {
    const data = await fetchJson<{ status: string; email?: string; message?: string }>(
      `${BASE_URL}/api/v2/oauth/session/status/${encodeURIComponent(sessionId)}`,
      { provider: "instantly", headers: authHeaders(), timeoutMs: 15_000 }
    );
    const state: InstantlyOAuthSessionState =
      data.status === "connected" ? "connected" : data.status === "failed" ? "failed" : "pending";
    return { state, email: data.email, message: data.message };
  },
  ...
  async listAccounts(): Promise<InstantlyAccount[]> {
    ...
    }>(`${BASE_URL}/api/v2/accounts`, { provider: "instantly", headers: authHeaders(), timeoutMs: 15_000 });
  ...
  async setWarmup(email: string, enabled: boolean): Promise<void> {
    await fetchJson(`${BASE_URL}/api/v2/accounts/${encodeURIComponent(email)}/warmup`, {
      provider: "instantly",
      method: "POST",
      headers: authHeaders(),
  ...
  async createCampaign(name: string, senderEmail: string): Promise<string> {
    const data = await fetchJson<{ id: string }>(`${BASE_URL}/api/v2/campaigns`, {
      provider: "instantly",
      method: "POST",
      headers: authHeaders(),
      body: JSON.stringify({ name, email_list: [senderEmail] }),
      timeoutMs: 20_000,
    });
    return data.id;
  },

  /** Adds a single approved lead + first-touch message to an Instantly campaign. */
  async addLead(
    campaignId: string,
    lead: { email: string; firstName?: string; subject: string; body: string }
  ): Promise<void> {
    await fetchJson(`${BASE_URL}/api/v2/leads`, {
      ...
      body: JSON.stringify({
        ...
        custom_variables: { subject: lead.subject, first_touch_body: lead.body },
      }),
      timeoutMs: 20_000,
    });
  },

  /** Starts sending for a campaign. Reigna only calls this after human approval. */
  async activateCampaign(campaignId: string): Promise<void> {
    await fetchJson(`${BASE_URL}/api/v2/campaigns/${campaignId}/activate`, {
      provider: "instantly",
      method: "POST",
      headers: authHeaders(),
      timeoutMs: 15_000,
  ...
  async pauseCampaign(campaignId: string): Promise<void> {
    await fetchJson(`${BASE_URL}/api/v2/campaigns/${campaignId}/pause`, {
      provider: "instantly",
      method: "POST",
      headers: authHeaders(),
      timeoutMs: 15_000,
 ...
 * secret rather than assuming an HMAC signature scheme this codebase can't
 * verify against undocumented behavior. Configure the same value in both
 * `INSTANTLY_WEBHOOK_SECRET` and the webhook's custom header in Instantly's
 * dashboard (`x-reigna-webhook-secret`). See /docs/API_INTEGRATION.md.
 */
export function verifyInstantlyWebhook(headerValue: string | null): boolean {
